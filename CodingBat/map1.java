...
// set the key "a" to have the value "". Basically "b" is a bully, taking the value and replacing it with the empty string.
...
//   Given a map of food keys and topping values, modify and return the map as follows: if the key "potato" has a value, set that as the value for the key "fries". If the key "salad" has a value,
...
//   Modify and return the given map as follows: if the keys "a" and "b" have values that have different lengths, then set "c" to have the longer value. If the values exist and have the same length,
//   change them both to the empty string in the map.  

  public Map<String, String> mapAB4(Map<String, String> map) {
    int a_len = map.containsKey("a")? map.get("a").length():-1;
    int b_len = map.containsKey("b")? map.get("b").length():-1;
    if(a_len!=-1  && b_len!=-1 ){
    if (b_len == a_len){
      map.put("a","");
      map.put("b","");
    }else{
        String c = a_len>b_len?map.get("a"):map.get("b");
        map.put("c",c);
    }
    }
    return map;
